...
Steps 1-4 prove the services are up; steps 5-7 are what prove Astra is usable.
The CLI defaults to `http://127.0.0.1:17001`; set `ASTRA_API_URL` or pass
`--api-url` if you remapped `ASTRA_API_PORT`. `model check` reports
`is_active` and `connectivity`, and a model becomes active only when the probe
succeeds.
