...
impl PostLoopPersistContext {
    ...
    pub(crate) async fn run_after_core(
        ...
        // and write independent sinks. Callers may run this projection phase
        // after publishing the already durable terminal boundary; the writes
        // here are still awaited together before lifecycle cleanup completes.
        ...
        let context_trace_persist =
            crate::turn::agentic_loop::finalization::persist_deferred_context_trace_signal(
                self.session_id.clone(),
                state.telemetry.context_trace_persistence.clone(),
                state.telemetry.pending_context_trace_signal.clone(),
            );
    ...
    pub(crate) async fn persist_core_and_trace_in_transaction(
        &self,
        state: &AgenticLoopState,
        user_transcript_precommitted: bool,
    ) -> Result<(), String> {
        ...
        // Core and detail events share one idempotency read, one batched event
        // write, one batched edge write, and one session-count update. They stay
        // in this transaction with transcript items, so replay never observes a
        // partial turn.
        if let Err(error) = persist_server_loop_core_and_trace_events_in_tx(
            &mut tx,
            &self.user_id,
            &self.session_id,
            &self.run_id,
        ...
        .await
        {
            let msg = format!("core/detail events tx failed: {error}");
            tracing::warn!(
                session_id = %self.session_id,
                error = %error,
                "post-loop: core/detail events tx failed, rolling back MO transaction"
            );
        ...
        let transcript_user_message = if user_transcript_precommitted {
            ""
        } else {
            &self.user_message
        };
...
fn build_server_loop_core_events(
    ...
    if let Some(event) = user_query_event {
        events.push(event);
    }
    events.extend(user_intent_events);
    if let Some(event) = llm_response_event {
        events.push(event);
    }
    events
}
...
impl TranscriptReasoningProjection {
    ...
    fn is_empty(&self) -> bool {
        // A completion marker without reasoning text carries no transcript
        // payload: reasoning_status is only meaningful when reasoning exists.
        // Avoid rewriting the assistant item and its transcript page for this
        // no-op case.
        self.text.is_empty()
    }
}
...
pub(crate) async fn materialize_server_run_transcript_evidence(
    ...
    let mut terminal_assistant = terminal_assistant;
    let terminal_assistant_source_event_id = terminal_assistant
        .as_ref()
        .map(|assistant| assistant.source_event_id.clone());
    if !reasoning.is_empty()
        && let Some(assistant) = terminal_assistant.as_mut()
    {
        let payload = assistant.payload.get_or_insert_default();
        payload.reasoning = Some(reasoning.text.clone());
        payload.reasoning_status = Some(if reasoning.done {
            "complete".to_string()
        } else {
            "streaming".to_string()
        });
    ...
    let mut tx = pool
        ...
        .map_err(|error| error.to_string())?;
    let persist_outcome =
        match persist_session_transcript_items_inner_in_tx(&mut tx, user_id, session_id, &items)
            .await
        {
            Ok(outcome) => outcome,
            Err(error) => {
    ...
    let assistant_inserted_with_reasoning = terminal_assistant_source_event_id
        .as_ref()
        .is_some_and(|source_event_id| {
            persist_outcome
                .inserted_source_event_ids
                .contains(source_event_id)
        });
    if !assistant_inserted_with_reasoning
        && let Err(error) = update_run_assistant_transcript_reasoning_in_tx(
...
#[allow(clippy::too_many_arguments)]
pub(crate) async fn persist_server_loop_core_and_trace_events_in_tx(
    tx: &mut sqlx::Transaction<'_, sqlx::MySql>,
    user_id: &str,
    session_id: &str,
    run_id: &str,
    ...
    DatabaseTraceEventWriter::write_many_in_tx(tx, events)
        .await
        .map_err(|error| error.to_string())
}
...
pub(crate) struct TranscriptPersistOutcome {
    inserted_source_event_ids: HashSet<String>,
}
...
pub(crate) async fn persist_session_transcript_items_inner_in_tx(
    ...
    session_id: &str,
    items: &[TranscriptPersistItem],
) -> Result<TranscriptPersistOutcome, sqlx::Error> {
    if items.is_empty() {
        return Ok(TranscriptPersistOutcome::default());
    }
    ...
    // One indexed statement loads both the next ordered sequence and every
    // idempotency key already present in this batch. The session row lock
    // above remains the serialization authority for transcript writers, so
    // this removes N per-item existence round trips without weakening order
    // or replay safety.
    let mut lookup = sqlx::QueryBuilder::<sqlx::MySql>::new(
        "SELECT seq_state.next_seq, existing.source_event_id
         FROM (
             SELECT COALESCE(MAX(item_seq), 0) + 1 AS next_seq
             FROM session_transcript_items
             WHERE session_id = ",
    );
    lookup.push_bind(session_id);
    lookup.push(" AND user_id = ");
    lookup.push_bind(user_id);
    lookup.push(
        ") AS seq_state
         LEFT JOIN session_transcript_items AS existing
           ON existing.session_id = ",
    );
    lookup.push_bind(session_id);
    lookup.push(" AND existing.user_id = ");
    lookup.push_bind(user_id);
    lookup.push(" AND existing.source_event_id IN (");
    {
        let mut separated = lookup.separated(", ");
        for item in items {
            separated.push_bind(&item.source_event_id);
        }
        separated.push_unseparated(")");
    }
    let rows = lookup.build().fetch_all(&mut **tx).await?;
    let mut next_seq = rows
        .first()
        .ok_or_else(|| {
            sqlx::Error::Protocol(
                "transcript sequence lookup returned no aggregate row".to_string(),
            )
        })?
        .try_get::<i64, _>("next_seq")?;
    let existing_source_event_ids = rows
        .iter()
        .map(|row| row.try_get::<Option<String>, _>("source_event_id"))
        .collect::<Result<Vec<_>, _>>()?
        .into_iter()
        .flatten()
        .collect::<HashSet<_>>();
    let mut outcome = TranscriptPersistOutcome::default();
    let mut dirty_pages = BTreeSet::new();

    for item in items {
        if existing_source_event_ids.contains(&item.source_event_id) {
            continue;
        }
        ...
        outcome
            .inserted_source_event_ids
            .insert(item.source_event_id.clone());
...
mod tests {
    ...
    #[test]
    fn reasoning_completion_without_text_has_no_transcript_projection() {
        let mut reasoning = TranscriptReasoningProjection::default();
        apply_reasoning_event_payload(&mut reasoning, &json!({"type": "reasoning_done"}));
    ...
    async fn core_persistence_records_deferred_inputs_without_splitting_audit_turns() {
        ...
        persist
            .persist_core_and_trace_in_transaction(&state, false)
            .await
